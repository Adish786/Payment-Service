# Spring Boot "Payment-Service Microservice"
...
```
2017-08-29 17:31:23.091  INFO 19387 --- [           main] s.b.c.e.t.TomcatEmbeddedServletContainer : Tomcat started on port(s): 8003 (http)
2017-08-29 17:31:23.097  INFO 19387 --- [           main] com.khoubyari.example.Application        : Started Application in 22.285 seconds (JVM running for 23.032)
```

## About the Service

The service is just a simple Online Shopping portal  review REST service. It uses an in-memory database (H2) to store the data. You can also do with a relational database like MySQL or PostgreSQL. If your database connection properties work, you can call some REST endpoints defined in ```com.khoubyari.example.api.rest.hotelController``` on **port 8003**. (see below)

More interestingly, you can start calling some of the operational endpoints (see full list below) like ```/metrics``` and ```/health``` (these are available on **port 8003**)
...
```
http://localhost:8003/payments  //Get All Record from the database
http://localhost:8003/paymentById/21 //Get partical id from the database
http://localhost:8003/payment/Paypal  //Get records by name 
```

### Create a payment resource

```
POST http://localhost:8003/addPayment  //Created only single payment
...
POST http://localhost:8003/addPayments  //Created Multiple payment
Accept: application/json
Content-Type: application/json

[
    {
     "paymentserviceprovider" : "PayUmoney",
   "totalpayout" : 100,
   "quantity" : 108
},
{
    "paymentserviceprovider" : "CCAvenue",
   "totalpayout" : 3000,
   "quantity" : 104
}
...
### Retrieve a paginated list of payment
...
```
PUT http://localhost:8003/update
Accept: application/json
Content-Type: application/json
...
Run the server and browse to localhost:8003/postman-ui.html
